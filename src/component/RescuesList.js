...
import { rescuesApi } from '../rest/rescuesApi';
...
export class RescuesList extends React.Component {
    ...
    fetchRescues = async () => {
        try {
            const rescues = await rescuesApi.get();
    ...
    updateRescue = async (updatedRescue) => {
        try {
            await rescuesApi.put(updatedRescue);
            this.fetchRescues();
        } catch (error) {
    ...
    createRescue = async () => {
        try {
            ...
            const createdRescue = await rescuesApi.create(newRescue);
            this.setState({
    ...
    deleteRescue = async (rescueId) => {
        try {
            await rescuesApi.delete(rescueId, true);
